...
- 独立的 `level2_validation.py` 可对上述四类日级事件做 1／3／5 交易日收盘到收盘后续收益研究：明确训练截止、最长预测期限隔离区、样本外事件、未到期和缺价格；对照为同日可比较股票等权收益。Vue“后续验证”页签按需运行并展示描述统计，已加载摘要可凭来源校验后随当前证据快照冻结；旧快照不补读。逐股明细暂仅可用命令行 `--details` 查看。它不接入实时排序，不能当作可成交收益或信号有效性证明。
