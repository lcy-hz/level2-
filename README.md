...
- Vue 原候选栏已与当前已应用的同日连续窗口联动：保留原类型／价格筛选，增加资金变化、连续性与连续指标正倒序；卡片最多补两行日级证据，详情顶部列出资金／成交额／价格水平、变化、窗口趋势、持续性、质量和反证。未应用窗口或快照未冻结窗口时不读取最新状态补齐。盘中守位／补单事件、行业／风格匹配对照、回撤、市场共同样本状态和含成交约束的策略验证尚未实现。连续状态不是吸筹确认或买卖指令。
