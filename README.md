...
- 独立的 `level2_validation.py` 可对上述四类日级事件做 1／3／5 交易日收盘到收盘后续收益研究：明确训练截止、最长预测期限隔离区、样本外事件、未到期和缺价格；对照为同日可比较股票等权收益。输出描述统计、按事件日等权的超额均值、逐日来源与价格覆盖。它不接入实时排序／快照，不能当作可成交收益或信号有效性证明。
...
脚本复用日级聚合缓存与已校准来源，只在相邻两交易日方向均有效时形成事件；缺档不跨越。训练事件的**最长**预测期限若跨训练截止日，统一归入隔离区；样本外从训练截止日后的交易日开始。统计日收盘到未来第 N 个交易日收盘，事件本身仅在触发日日级文件就绪后已知，因此这里不是可执行的入场回测。输出中的未到期、价格缺失和缺 Level-2 日期各自分列，不补零。复权因子的历史当时可得性、涨跌停成交、费用、滑点、容量及行业／风格控制仍未验收；只能作为描述性检验，不能据此升级为有效策略。`--details` 可展开逐股事件证据。
