"""Local daily candles, normalized to the report-date adjustment anchor.
...
SOURCE = PATHS['stk_factor_pro']
CALENDAR = PATHS['daily']
FIELDS = ['ts_code', 'trade_date', 'open', 'high', 'low', 'close', 'adj_factor']
...
def normalized_candles(frame, codes, dates, target):
    ...
    for code in sorted(codes):
        ...
        anchor = None if group is None else group[group.trade_date == target]
        if anchor is None or anchor.empty or not (0 < anchor.adj_factor.iloc[0] < float('inf')):
            series[code] = {'bars': [], 'reason': '缺少报告日有效复权因子，未回退为不复权'}
            continue
        factor = float(anchor.adj_factor.iloc[0])
        bars, rejected = [], 0
        for row in group.sort_values('trade_date').itertuples():
            values = [row.open, row.high, row.low, row.close, row.adj_factor]
            if not all(pd.notna(v) and 0 < v < float('inf') for v in values) or not (
                ...
                rejected += 1
                continue
            prices = [round(float(v) * row.adj_factor / factor, 4) for v in values[:4]]
            bars.append([positions[row.trade_date], *prices])
        series[code] = {'anchor': factor, 'bars': bars, 'rejected': rejected}
    return series
...
def build_bundle(codes, target, source=SOURCE, calendar=CALENDAR, count=60, include_volume=False):
    ...
    return {'source': str(source.resolve()), 'target': target, 'dates': dates,
            ...
            'method': 'stk_factor_pro OHLC × adj_factor当日 ÷ adj_factor报告日；元/股；成交量 vol 单位手，不复权；amount 千元转换为元',
            'series': series}
