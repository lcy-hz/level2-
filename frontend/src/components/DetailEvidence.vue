...
<template>
  <section class="detail-evidence" aria-label="按需 Level-2 深查">
    ...
    <template v-if="evidence">
      ...
      <p v-if="!evidence.legacySummary" class="footnote">盘口中间价：{{ evidence.quotePath ? evidence.quotePath.status === 'AVAILABLE' ? evidence.quotePath.method : quoteStatus[evidence.quotePath.status] || '盘口路径不可比较' : '此结果未保存盘口路径，不读取最新数据回填' }}；与主动净额并列仅供观察，不把价稳自动解释为被动承接或补单。</p>
