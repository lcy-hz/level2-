...
const saved = savedDetailEvidence(props.card, props.day)
const state = ref(props.frozen ? { status: saved ? 'done' : 'unavailable', message: saved?.legacySummary ? '快照已保存汇总表；完整新式盘口深查未保存' : saved ? '快照已保存该股按需深查对象；未保存的新指标仍不可用' : '快照未保存匹配本股本日的深查；不补算' } : { status: 'idle', message: saved?.legacySummary ? '报告已保存汇总表；正在查询新式深查缓存' : '尚未查询本地缓存' })
...
const auditTime = row => row.timeFieldStatus === 'MISSING_FIELD' ? '源时间字段缺失' : `${auditCount(row.timeEarlier)} / ${auditCount(row.timeSame)} / ${auditCount(row.timeLater)} / ${auditCount(row.timeComparable)} / ${auditCount(row.singleRowMatchedTrades)}`
const responseBp = value => !Number.isFinite(value) ? '不可比较' : Math.abs(value) < 0.0001 ? '0 bp' : `${value > 0 ? '+' : ''}${value.toFixed(Math.abs(value) < 0.01 ? 4 : 2)} bp`
const responseStatus = { INVALID_TRADE_CLOCK: '成交时钟异常', INVALID_TRADE_PRINT: '成交价量异常', INVALID_QUOTE_DATE: '快照日期不一致', INVALID_QUOTE_CLOCK: '快照时钟异常', DUPLICATE_QUOTE_TIME: '快照时间重复', NO_COMPARABLE_PAIRS: '没有可比较的区间' }
...
<template>
  <section class="detail-evidence" aria-label="按需 Level-2 深查">
    <div class="detail-heading"><h3>盘中与关联单深查</h3><button v-if="!frozen && !result && (!card.computedDetail || evidence?.legacySummary)" type="button" :disabled="busy" @click="calculate">{{ busy ? '计算中…' : state.status === 'error' ? '重试计算' : evidence?.legacySummary ? '计算新式深查' : '点击计算' }}</button></div>
    <p class="footnote" role="status">{{ state.message }}。{{ frozen ? '只读快照，不读取最新数据。' : '仅计算本股、当前报告日；结果缓存于本机。' }}</p>
    <template v-if="evidence">
      <p v-if="evidence.legacySummary" class="footnote">已保存汇总仅含五时段主动成交、关联编号分组和 order_raw 类型统计；逐笔路径、盘口结构、编号匹配审计未保存。{{ frozen ? '快照不补读当前数据。' : '如需新式深查，可另行计算；不改写报告原始汇总。' }}</p>
      ...
      <h4>盘中五时段</h4>
      <div v-if="evidence.legacySummary" class="table-scroll"><table><thead><tr><th>时段</th><th>成交额</th><th>主动净额</th><th>VWAP</th></tr></thead><tbody><tr v-for="row in evidence.segments || []" :key="row.s"><td>{{ row.s }}</td><td>{{ money(row.a) }}</td><td>{{ money(row.n) }}</td><td>{{ row.v ?? '未知' }}</td></tr></tbody></table></div>
      ...
      <template v-if="!evidence.legacySummary">
      ...
      <details v-if="evidence.priceResponse" class="book-observations"><summary>成交方向与相邻中间价 · 非因果冲击</summary><p class="footnote">{{ evidence.priceResponse.status === 'AVAILABLE' ? evidence.priceResponse.method : responseStatus[evidence.priceResponse.status] || '价格反应不可比较' }}</p><div v-if="evidence.priceResponse.segments?.length" class="table-scroll"><table><thead><tr><th>时段</th><th>有资金方向区间 / 相邻报价</th><th>同区间方向对齐均值 / 中位</th><th>同向变动占比</th><th>价格不变区间</th><th>下一快照方向对齐均值 / 中位</th><th>下一可比区间</th><th>缺口 / 无效报价 / 未知方向 / 零净额</th></tr></thead><tbody><tr v-for="row in evidence.priceResponse.segments" :key="row.s"><td>{{ row.s }}</td><td>{{ auditCount(row.flowPairs) }} / {{ auditCount(row.adjacentPairs) }}</td><td>{{ responseBp(row.meanAlignedBps) }} / {{ responseBp(row.medianAlignedBps) }}</td><td>{{ ratio(row.positiveAlignedSharePct) }}</td><td>{{ auditCount(row.zeroMovePairs) }}</td><td>{{ responseBp(row.meanNextAlignedBps) }} / {{ responseBp(row.medianNextAlignedBps) }}</td><td>{{ auditCount(row.nextPairs) }}</td><td>{{ auditCount(row.gapPairs) }} / {{ auditCount(row.invalidQuotePairs) }} / {{ auditCount(row.unknownFlowPairs) }} / {{ auditCount(row.zeroFlowPairs) }}</td></tr></tbody></table></div><p class="footnote">方向对齐值＝主动净额符号 × 中间价变化（bp）；“同向变动占比”的分母包含价格未变区间。后一快照期间仍可能有其他成交、挂撤单与外部信息，故不是单笔成交冲击、永久冲击或可交易预测。均值受极端值影响；中位数为0可能只表示多数快照价格不变。</p></details>
      <details v-if="evidence.quotePath?.source"><summary>盘口快照来源</summary><code>{{ evidence.quotePath.source }}</code></details>
      </template>
      <h4>主动成交关联编号分组（身份未验证）</h4>
      ...
      <details :open="evidence.legacySummary || undefined"><summary>order_raw 覆盖（非撤单结论）</summary><div class="table-scroll"><table><thead><tr><th>类型原值</th><th>方向</th><th>行数</th><th>数量</th></tr></thead><tbody><tr v-for="(row, index) in evidence.orders || []" :key="index"><td>{{ row.t }}</td><td>{{ row.s }}</td><td>{{ row.r?.toLocaleString() ?? '未知' }}</td><td>{{ row.q?.toLocaleString() ?? '未知' }}</td></tr></tbody></table></div></details>
      <template v-if="!evidence.legacySummary">
      <details v-if="evidence.orderLinkAudit?.length" class="book-observations"><summary>候选委托编号关联审计 · 非订单身份</summary><div class="table-scroll"><table><thead><tr><th>原始字段</th><th>状态</th><th>有效委托行 / 原始行</th><th>候选键 / 重复键组 / 多代码键组</th><th>字段交集成交 / 有效成交</th><th>委托代码同字母</th><th>单条键且同字母</th><th>重复键涉及成交</th><th>单条键时间：早 / 同 / 晚 / 可比 / 已关联</th></tr></thead><tbody><tr v-for="row in evidence.orderLinkAudit" :key="row.field"><td>{{ row.field }}</td><td>{{ auditStatus[row.status] || '不可比较' }}</td><td>{{ auditCount(row.validOrderRows) }} / {{ auditCount(row.orderRows) }}</td><td>{{ auditCount(row.candidateKeys) }} / {{ auditCount(row.repeatedKeyGroups) }} / {{ auditCount(row.multiCodeKeyGroups) }}</td><td>{{ auditCount(row.matchedTrades) }} / {{ auditCount(row.eligibleTrades) }}</td><td>{{ auditCount(row.sameCodeMatches) }}</td><td>{{ auditCount(row.singleRowSameCodeMatches) }}</td><td>{{ auditCount(row.repeatedKeyMatchedTrades) }}</td><td>{{ auditTime(row) }}</td></tr></tbody></table></div><p class="footnote">逐笔成交按主动方向选取叫买／叫卖序号，与两种原始委托编号分别做正编号字段交集；“同字母”仅比较 B/S 字段字符。时间关系仅对单条候选键、双方时钟有效的成交比较；同刻不能证明委托先于成交，较早也不能独立验证经济订单身份。重复键、多代码键、无效时钟及缺失数据保留歧义；不能据此认定新增、撤单、补单、被动吸筹或母单身份。</p></details>
      </template>
