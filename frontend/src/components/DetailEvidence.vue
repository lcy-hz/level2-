...
<template>
  <section class="detail-evidence" aria-label="按需 Level-2 深查">
    ...
    <template v-if="evidence">
      ...
      <h4>盘中五时段</h4>
      <div v-if="!evidence.quotePath" class="table-scroll"><table><thead><tr><th>时段</th><th>成交额</th><th>主动净额</th><th>VWAP</th></tr></thead><tbody><tr v-for="row in evidence.segments || []" :key="row.s"><td>{{ row.s }}</td><td>{{ money(row.a) }}</td><td>{{ money(row.n) }}</td><td>{{ row.v ?? '未知' }}</td></tr></tbody></table></div>
