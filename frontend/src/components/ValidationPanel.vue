    ...
    <template v-if="result">
      ...
      <section class="validation-limit-study" aria-label="涨停收盘股专项研究">
        ...
        <template v-if="limitStudy">
          ...
          <details class="validation-event validation-day-breakdown"><summary>次日开盘价格参考 · 非成交收益</summary>
            <p class="footnote">只对有成交量、价关系有效且复权日线齐全的次日日 K 计算开盘参考价；单一价位日保留为价格参考，不推断可排队买入。开盘缺失与目标日缺价不补零。“信号收盘→目标收盘”与“次日开盘→目标收盘”两列使用完全相同的逐股样本；开盘跳空列只要求次日开盘价，样本数可能不同。价格路径未扣费用、滑点，也没有委托成交证据。</p>
            <p v-if="selectedHorizon === 1" class="footnote">后 1 日：次日开盘到当日收盘不是普通 A 股可执行的当日买卖路径，此处不展示。</p>
            <p v-else-if="limitRows.some(row => row.entryOpenObserved == null)" class="footnote">旧研究未保存次日开盘价格参考；不读取最新数据补齐。</p>
            <div v-else class="table-scroll"><table class="validation-table"><thead><tr><th>市场／个股</th><th>开盘跳空 · 覆盖</th><th>开盘跳空均值</th><th>同样本信号收盘→目标收盘</th><th>次日开盘→目标收盘</th><th>配对覆盖／触发</th></tr></thead><tbody><tr v-for="row in limitRows" :key="row.marketDirection + row.stockDirection"><td>{{ directionName[row.marketDirection] }}／{{ directionName[row.stockDirection] }}</td><td>{{ row.entryOpenObserved }} / {{ row.events }}</td><td>{{ formatPct(row.meanSignalToNextOpenPct) }}</td><td>{{ formatPct(row.meanSignalToTargetPairedPct) }}</td><td>{{ formatPct(row.meanNextOpenToTargetPct) }}</td><td>{{ row.postOpenObserved }} / {{ row.events }}</td></tr></tbody></table></div>
            <p class="footnote">普通沪深 A 股买入当日不能卖出；后 {{ selectedHorizon }} 日的终点收盘价同样只是价格参照，不是保证卖出的成交价。本表用于检查原收盘后收益中有多少发生在次日开盘之前，不代表策略净收益。</p>
          </details>
