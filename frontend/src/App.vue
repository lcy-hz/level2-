...
async function load() {
  ...
  try {
    ...
    activeTab.value = result.patternUI?.tab === 'patterns' ? 'patterns' : 'review'
...
async function save() {
  ...
  try {
    ...
    saveMessage.value = `已保存 ${savedSnapshot.day} 快照：${savedSnapshot.chartCount} 个图表、${Object.keys(loadedDetails.value).length} 只已加载深查、${Object.keys(stateReceipts.value).length} 个连续窗口、${patternReceipt.value ? '形态结果及' + patternCodes.value.length + '张逐根图' : '无形态结果'}。`
...
<template>
  <main class="shell">
    ...
    <template v-else-if="document">
      ...
      <nav class="research-tabs" role="tablist" aria-label="研究视图"><button role="tab" :aria-selected="activeTab === 'review'" aria-controls="review-panel" @click="activeTab = 'review'">Level‑2 复盘</button><button role="tab" :aria-selected="activeTab === 'patterns'" aria-controls="patterns-panel" @click="activeTab = 'patterns'">个股形态</button></nav>
      <div id="review-panel" role="tabpanel" v-show="activeTab === 'review'"><QualitySummary :quality="document.report.quality" :gate="document.report.gate" /><MarketTimeline :markets="document.report.markets" /><ContinuousPanel :key="document.mode + document.day" :day="document.day" :cards="document.report.cards" :frozen="document.mode === 'snapshot'" :snapshot-views="document.stateViews || {}" :snapshot-window="document.stateWindow" :snapshot-charts="document.charts || {}" @chart-loaded="captureChart" @state-loaded="captureState" @view-applied="activeStateView = $event.view" /><CandidatePanel :key="document.mode + document.day" :cards="document.report.cards" :day="document.day" :frozen="document.mode === 'snapshot'" :snapshot-charts="document.charts || {}" :initial-filters="filters" @chart-loaded="captureChart" @detail-loaded="captureDetail" @filters-change="filters = $event" /></div>
      ...
      <section class="panel migration-note"><h2>研究边界</h2><p>Vue 已接入日级报告、连续观察、单股深查、22 类形态及所查看证据的快照。形态规则未经收益校准；盘口队列重建、事件后验验证与交易授权仍未实现。旧页保留为迁移对照。</p></section>
    </template>
  </main>
</template>
