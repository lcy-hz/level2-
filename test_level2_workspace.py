...
class WorkspaceTests(unittest.TestCase):
    ...
    def test_snapshot_freezes_only_verified_full_detail(self):
        detail={'code':'000001.SZ','day':'20260907','tradeRows':42,'amount':1000,'net':30,
                ...
                'orderLinkAudit':[{'field':'交易所委托号','status':'AVAILABLE','eligibleTrades':42,
                                   'matchedTrades':24,'sameCodeMatches':20,'singleRowMatchedTrades':19,
                                   'timeComparable':18,'timeEarlier':2,'timeSame':16,'timeLater':0}],
                'quotePath':{'status':'AVAILABLE','source':'/test/snapshot.parquet',
                             'segments':[{'s':'09:30–10:00','midChangePct':.5,'valid':20,'observed':20,
                                          ...
                                          'bidDisplayedRecovery':{'status':'OBSERVED','samePricePairs':10,
                                                                  'declines':2,'recovered':1,'censored':1,
                                                                  'medianObservedSeconds':3}}]},
    ...
    def test_snapshot_freezes_verified_validation_without_live_backfill(self):
        study={'start':'20260901','end':'20260907','asof':'20260907','split':'20260905',
               'summary':[{'rule':'SELL_EASING','observed':12,'triggerDays':1,'entryGateCounts':{'PRICE_REFERENCE_ONLY':10,'ONE_PRICE_SESSION':2},
                           'closeDrawdownObserved':11,'medianMaxCloseDrawdownPct':-2.5,
                           'signalDayBreakdown':[{'day':'20260907','events':12,'observed':12,'pending':0,
                                                  'missingPrice':0,'meanExcessPct':1.2}]}],
        ...
        self.assertEqual(frozen['validation']['summary'][0]['signalDayBreakdown'][0]['day'],'20260907')
