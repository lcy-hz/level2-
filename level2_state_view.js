...
function derive(result){
 ...
 const trajectory=dates.map(day=>{const amounts=rows.map(s=>s.byDay[day]).filter(r=>r&&finite(r.amount)&&r.amount>0),flow=common.map(s=>s.byDay[day]),den=flow.reduce((a,r)=>a+r.amount,0),net=flow.reduce((a,r)=>a+r.net,0);return {day,amount:amounts.length?amounts.reduce((a,r)=>a+r.amount,0):null,amountCoverage:amounts.length,directionCoverage:rows.filter(s=>usable(s.byDay[day])).length,commonAmount:flow.length?den:null,commonNet:flow.length?net:null,ratio:den?100*net/den:null,buyShare:flow.length?100*flow.filter(r=>r.ratio>0).length/flow.length:null,sourceStatus:sources[day]?.status||null,sourceFile:sources[day]?.source||null};});
 trajectory.forEach((row,index)=>{const previous=trajectory[index-1];row.ratioDeltaPP=previous&&finite(row.ratio)&&finite(previous.ratio)?row.ratio-previous.ratio:null;row.buyShareDeltaPP=previous&&finite(row.buyShare)&&finite(previous.buyShare)?row.buyShare-previous.buyShare:null;});
 ...
 return {method:'common-cohort-weighted-flow-and-breadth',eventMethod:result.eventMethod||null,day:result.day,window:result.window,dates,total:rows.length,common:common.length,trajectory,marketState:marketState(trajectory,result.window,common.length),applicable,counts,stocks};
}
